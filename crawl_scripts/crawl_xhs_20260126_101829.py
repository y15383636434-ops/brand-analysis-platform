...
类型: creator
关键词: N/A
目标URL: https://www.douyin.com/user/MS4wLjABAAAAEFr-j40QINDuTpyyq_pxDK1TqMQoU_sT-VFVSdkojdc?from_tab_name=main
最大数量: 3
生成时间: 2026-01-26 10:18:29
"""
# -*- coding: utf-8 -*-
import subprocess
import sys
...
# 命令参数
cmd = ['C:\\Users\\Yu\\cursorProjects\\githup\\MediaCrawler\\python_env\\python.exe', 'C:\\Users\\Yu\\cursorProjects\\githup\\MediaCrawler\\main.py', '--platform', 'xhs', '--lt', 'cookie', '--type', 'creator', '--save_data_option', 'json', '--keywords', 'MS4wLjABAAAAEFr-j40Q', '--note_type', 'video', '--get_comment', 'no', '--get_sub_comment', 'no']
...
print(f"开始爬取: xhs")
print(f"类型: creator")
print(f"目标URL: https://www.douyin.com/user/MS4wLjABAAAAEFr-j40QINDuTpyyq_pxDK1TqMQoU_sT-VFVSdkojdc?from_tab_name=main")
print(f"最大数量: 3")
print("=" * 70)
...
config_file_path = mediacrawler_path / "config" / "xhs_config.py"
...
try:
    if config_file_path.exists():
        ...
        # 替换变量值
        target_urls = ['https://www.douyin.com/user/MS4wLjABAAAAEFr-j40QINDuTpyyq_pxDK1TqMQoU_sT-VFVSdkojdc?from_tab_name=main']
        
        # 构造新的赋值语句
        new_assignment = f'XHS_CREATOR_ID_LIST = {target_urls}'
        
        # 使用正则替换整个列表定义
        # 匹配 XHS_CREATOR_ID_LIST = [ ... ] (包括多行)
        ...
        if match:
            ...
            print(f"已更新配置变量 XHS_CREATOR_ID_LIST")
            print(f"新内容(前100字符): {new_assignment[:100]}...")
        else:
            print(f"警告: 未在配置文件中找到变量 XHS_CREATOR_ID_LIST，尝试直接追加")
