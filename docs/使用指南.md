# 📖 品牌分析系统 - 使用指南

## 🚀 快速开始

### 1. 启动服务

**方法1：使用批处理文件（推荐）**
- 双击运行：`启动完整系统.bat`

**方法2：手动启动**
- 启动Celery Worker：双击 `启动Celery.bat`
- 启动FastAPI应用：双击 `启动FastAPI.bat`

### 2. 访问API文档

打开浏览器访问：**http://localhost:8000/docs**

---

## 📋 功能使用

### 品牌管理

#### 创建品牌
```
POST /api/v1/brands
{
  "name": "品牌名称",
  "description": "品牌描述",
  "keywords": ["关键词1", "关键词2"],
  "platforms": ["xhs", "douyin", "weibo", "zhihu"]
}
```

#### 查看品牌列表
```
GET /api/v1/brands?page=1&page_size=10
```

#### 查看品牌详情
```
GET /api/v1/brands/{brand_id}
```

---

### 数据采集

#### 启动爬虫任务
```
POST /api/v1/brands/{brand_id}/crawl
{
  "platforms": ["xhs", "douyin", "weibo"],
  "keywords": ["关键词"],
  "max_items": 100,
  "include_comments": true
}
```

**支持的平台**:
- `xhs` - 小红书
- `douyin` - 抖音
- `weibo` - 微博
- `zhihu` - 知乎
- `bilibili` - B站
- `kuaishou` - 快手

#### 查看爬虫任务
```
GET /api/v1/crawl-tasks?brand_id=1&status=completed
```

---

### 数据查看

#### 查看品牌数据
```
GET /api/v1/brands/{brand_id}/data?platform=xhs&page=1&page_size=20
```

#### 查看数据统计
```
GET /api/v1/brands/{brand_id}/data/stats
```

#### 删除数据
```
DELETE /api/v1/brands/{brand_id}/data?platform=xhs
```

---

### AI分析

#### 启动分析任务
```
POST /api/v1/brands/{brand_id}/analyze
{
  "analysis_type": "comprehensive",
  "include_sentiment": true,
  "include_keywords": true,
  "include_insights": true
}
```

#### 查看分析结果
```
GET /api/v1/brands/{brand_id}/analysis
```

---

## 📊 数据存储

### 数据位置

- **MySQL**: 结构化数据（品牌、任务、报告）
- **MongoDB**: 非结构化数据（爬取内容、分析结果）
  - 数据库：`brand_analysis`
  - 集合：`raw_data`（爬取数据）、`analysis_results`（分析结果）

### 查看数据

**方法1：使用API**
- 访问 http://localhost:8000/docs
- 使用数据查看API

**方法2：使用脚本**
```bash
python view_crawled_data.py 1
```

**方法3：使用MongoDB客户端**
- 连接到 `mongodb://localhost:27017`
- 查看 `brand_analysis.raw_data` 集合

---

## ⚙️ 配置说明

### LLM API配置（可选）

在 `.env` 文件中配置：

```env
# OpenAI配置
OPENAI_API_KEY=your_key
OPENAI_MODEL=gpt-4

# 或Claude配置
ANTHROPIC_API_KEY=your_key
ANTHROPIC_MODEL=claude-3-5-sonnet-20241022
```

**注意**：如果不配置LLM API，基础分析（情感、关键词）仍可用，但深度洞察功能不可用。

---

## 🔍 常见问题

### Q: 第二次爬取会报错吗？

**A**: 不会。系统已实现自动去重，第二次爬取会更新数据，不会报错。

### Q: 数据存储在哪里？

**A**: 数据存储在MongoDB的`raw_data`集合中。可以通过API或脚本查看。

### Q: 如何查看爬取的数据？

**A**: 有三种方法：
1. 使用API：`GET /api/v1/brands/{brand_id}/data`
2. 使用脚本：`python view_crawled_data.py {brand_id}`
3. 使用MongoDB客户端直接查看

---

## 📚 详细文档

- **多平台爬取**: `多平台爬取使用说明.md`
- **数据存储**: `数据存储和查看说明.md`
- **AI分析**: `AI分析功能实现总结.md`
- **服务启动**: `服务启动说明.md`
- **API测试**: `Swagger UI使用指南.md`

---

**详细使用说明请参考相关文档！** 📖






